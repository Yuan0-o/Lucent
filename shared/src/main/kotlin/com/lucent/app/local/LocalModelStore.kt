 ...
 *
 * P1-1: same seam as [com.lucent.app.data.FontStore] — the only platform difference was ever where
 * a picked file's bytes come from. [PlatformModelSource] (a typealias per platform, in each
 * module's own `PlatformModel.kt`) plus [openModelSource] / [modelSourceDisplayName] cover both
 * [import] and [importMmproj]; everything else below was always platform-neutral.
 */
object LocalModelStore {
    ...
    /**
     * Import the model at [source] into a NEW slot and make it active. Blocking I/O — call from a
     ...
     *
     * [source] is a [PlatformModelSource] — an Android content `Uri` or a desktop `File` depending
     * on which module this compiles into (see `PlatformModel.kt` in each). Everything from here
     * down — including the zip-unwrap — is the same pipeline on both platforms.
     *
     ...
     * Same [PlatformModelSource] seam as [import] — an Android `Uri` or a desktop `File`.
     */
