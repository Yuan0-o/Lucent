 ...
 *
 * P1-1: the only platform difference was ever *where the picked file's bytes come from* — an
 * Android content [android.net.Uri] versus a desktop [java.io.File]. That's [PlatformFontSource]
 * (a typealias per platform, in each module's own `PlatformFont.kt`), plus the two tiny functions
 * that read from it. Everything below — the manifest, the magic-byte check, the temp-file-then-
 * rename write, backup support — was always platform-neutral and now lives in one place.
 */
object FontStore {
    ...
    /**
     * Import the font at [source] into a NEW slot. Blocking I/O — call from a background
     * dispatcher. On success a new slot exists, named [customName] (or the picked file's name when
     * that is blank). Making it the *selected* font is the caller's decision, since the selection
     * lives in settings, not here.
     *
     * [source] is a [PlatformFontSource] — an Android content `Uri` or a desktop `File` depending
     * on which module this compiles into (see `PlatformFont.kt` in each). Everything from here down
     * is the same verify/copy pipeline on both platforms.
     *
