...
object LocalModelStore {
    ...
    // Desktop adaptation: the source is a plain java.io.File from the system file dialog
    // instead of an Android content resolver stream; the copy/verify/zip-extract pipeline
    // below is untouched.
    fun import(context: Context, source: File, customName: String? = null): ModelSlot {
        ...
        var pickedName = source.name.ifBlank { "model.gguf" }
     ...
     * Desktop takes a plain [File] (the AWT picker returns one) — same seam shape as [import].
     */
    fun importMmproj(context: Context, id: String, source: File): File {
        val dir = dir(context)
        if (!dir.exists() && !dir.mkdirs()) throw IOException("Could not create model directory")
