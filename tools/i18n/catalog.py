...
ENTRIES = [
    ...
    ("themeMonetRoseDesc", "Soft pale pink", "柔和的淡粉色", "やわらかな淡いピンク", "부드러운 연분홍"),
    ...
    ("themeMonetInkDesc", "A deep inky teal", "深邃的青墨色", "深いインクのティール", "짙은 잉크빛 청록"),
    ("themeMonetGarnet", "Garnet", "石榴", "ガーネット", "가넷"),
    ("themeMonetGarnetDesc", "A deep garnet crimson", "深邃的绯红", "深いクリムゾン", "짙은 크림슨"),
    ...
    ("themeSectionLight", "Light themes", "浅色主题", "ライトテーマ", "라이트 테마"),
    ("themeSectionDark", "Dark themes", "深色主题", "ダークテーマ", "다크 테마"),
    ("themeSectionSystem", "System themes", "系统主题", "システムテーマ", "시스템 테마"),
    ("themeMonetWheat", "Wheat", "麦田", "麦畑", "밀밭"),
    ("themeMonetWheatDesc", "Pale straw", "浅浅的麦秆色", "淡い麦わら色", "옅은 밀짚색"),
    ("themeMonetGarden", "Garden", "花园", "庭園", "정원"),
    ("themeMonetGardenDesc", "Pale water-garden green", "浅浅的水苑绿", "淡い水の庭の緑", "옅은 물의 정원 초록"),
    ("themeMonetMorning", "Morning", "晨光", "朝", "아침"),
    ("themeMonetMorningDesc", "Pale morning blue", "浅浅的晨曦蓝", "淡い朝の青", "옅은 아침 파랑"),
    ("themeMonetWisteria", "Wisteria", "紫藤", "藤", "등나무"),
    ("themeMonetWisteriaDesc", "Pale wisteria", "浅浅的紫藤色", "淡い藤色", "옅은 등나무색"),
    ("themeMonetNight", "Nightfall", "夜幕", "宵闇", "야경"),
    ("themeMonetNightDesc", "Deep evening blue", "深邃的傍晚蓝", "深い宵の青", "깊은 저녁 블루"),
    ("themeMonetPine", "Deep garden", "深园", "深緑の庭", "깊은 정원"),
    ("themeMonetPineDesc", "Deep garden green", "深邃的花园绿", "深い庭の緑", "깊은 정원 그린"),
    ("themeMonetPlum", "Dusk wisteria", "暮紫", "暮れの藤", "황혼 등나무"),
    ("themeMonetPlumDesc", "Deep wisteria plum", "深邃的紫藤紫", "深い藤紫", "깊은 등나무 자주"),
    ("themeMonetEmber", "Embers", "余烬", "残り火", "잔불"),
    ("themeMonetEmberDesc", "Deep haystack amber", "深邃的麦垛琥珀", "深い干し草の琥珀", "깊은 건초 앰버"),
    ...
    ("settingsAssistantSub", "Personalisation, the cloud model, the local model", "个性化、云端模型、本地模型", "パーソナライズ・クラウドモデル・ローカルモデル", "개인화, 클라우드 모델, 로컬 모델"),
    ...
    ("settingsPersonalizationSub", "Name, style, and how it behaves", "名称、风格与行为习惯", "名前・スタイル・振る舞い", "이름, 스타일, 행동 방식"),
    ...
    ("memoryCostTitle", "Memory", "记忆", "記憶", "기억"),
    ...
    ("apiProviderTitle", "Provider", "服务商", "プロバイダー", "제공업체"),
    ("apiProviderChatGpt", "ChatGPT", "ChatGPT", "ChatGPT", "ChatGPT"),
    ("apiProviderGemini", "Gemini", "Gemini", "Gemini", "Gemini"),
    ("apiProviderClaude", "Claude", "Claude", "Claude", "Claude"),
    ("apiProviderDeepSeek", "DeepSeek", "DeepSeek", "DeepSeek", "DeepSeek"),
    ("apiProviderKimi", "Kimi", "Kimi", "Kimi", "Kimi"),
    ("apiProviderCustom", "Custom", "自定义", "カスタム", "사용자 지정"),
    ("apiProviderBuiltInHint", "This address is built in — just add your API key. Choose Custom to enter your own address.", "此地址已内置——只需填写 API 密钥。需要自定义地址请选择「自定义」。", "このアドレスは組み込み済みです。APIキーを入力するだけで使えます。独自のアドレスを使うには「カスタム」を選んでください。", "이 주소는 기본 제공됩니다. API 키만 입력하면 됩니다. 직접 주소를 쓰려면 '사용자 지정'을 선택하세요."),
    ("apiModelsTitle", "Choose models", "选择模型", "モデルを選択", "모델 선택"),
    ("apiModelsCount(chosen: Int, total: Int)", "{chosen} of {total} selected", "已选 {chosen}/{total}", "{total} 件中 {chosen} 件を選択", "{total}개 중 {chosen}개 선택"),
    ("apiModelsEmpty", "The API returned no models.", "该 API 没有返回任何模型。", "APIがモデルを返しませんでした。", "API가 모델을 반환하지 않았습니다."),
    ("apiModelsFetchFailed(detail: String)", "Couldn't fetch the model list: {detail}", "获取模型列表失败：{detail}", "モデル一覧を取得できませんでした：{detail}", "모델 목록을 가져오지 못했습니다: {detail}"),
    ...
    ("dynamicColorSub", "Match your wallpaper and system accents (Android 12+).", "跟随你的壁纸与系统强调色（Android 12+）。", "壁紙とシステムのアクセントカラーに合わせます（Android 12 以上）。", "배경화면과 시스템 강조색에 맞춥니다(Android 12 이상)."),
    ...
    ("ccCreateNotebook(title: String)", "Create the notebook \"{title}\"", "创建笔记本“{title}”", "ノートブック「{title}」を作成", "노트북 \"{title}\" 만들기"),
    ("ccReadNotebook(title: String)", "Look through the notebook \"{title}\"", "查看笔记本“{title}”", "ノートブック「{title}」を確認", "노트북 \"{title}\" 살펴보기"),
    ("ccUpdateNotebook(title: String)", "Update the notebook \"{title}\"", "修改笔记本“{title}”", "ノートブック「{title}」を変更", "노트북 \"{title}\" 수정"),
    ("ccReadFormatting(title: String)", "Check the formatting of \"{title}\"", "查看“{title}”的格式", "「{title}」の書式を確認", "\"{title}\" 서식 확인"),
    ("ccRenameNotebook(title: String, newTitle: String)", "Rename the notebook \"{title}\" to \"{newTitle}\"", "将笔记本“{title}”重命名为“{newTitle}”", "ノートブック「{title}」を「{newTitle}」に名称変更", "노트북 \"{title}\"을(를) \"{newTitle}\"(으)로 이름 변경"),
    ("ccAddToNotebook(item: String, notebook: String)", "Add \"{item}\" to the notebook \"{notebook}\"", "将“{item}”加入笔记本“{notebook}”", "「{item}」をノートブック「{notebook}」に追加", "\"{item}\"을(를) 노트북 \"{notebook}\"에 추가"),
    ("ccRemoveFromNotebook(item: String, notebook: String)", "Remove \"{item}\" from the notebook \"{notebook}\"", "将“{item}”从笔记本“{notebook}”中移除", "「{item}」をノートブック「{notebook}」から削除", "\"{item}\"을(를) 노트북 \"{notebook}\"에서 제거"),
    ("ccMoveToNotebook(item: String, to: String)", "Move \"{item}\" to the notebook \"{to}\"", "将“{item}”移动到笔记本“{to}”", "「{item}」をノートブック「{to}」へ移動", "\"{item}\"을(를) 노트북 \"{to}\"(으)로 이동"),
    ...
    ("quickModelSearch", "Search models", "搜索模型", "モデルを検索", "모델 검색"),
    ("quickModelUseNamed(name: String)", "Use \u201c{name}\u201d", "使用\u201c{name}\u201d", "\u201c{name}\u201d を使う", "\u201c{name}\u201d 사용"),
    ...
    ("richTextSize", "Size", "字号", "文字サイズ", "글자 크기"),
    ("richTextSizeDefault", "Normal", "正常", "標準", "보통"),
    ("richTextSizeSmall", "Small", "小", "小", "작게"),
    ("richTextSizeMedium", "Medium", "中", "中", "중간"),
    ("richTextSizeLarge", "Large", "大", "大", "크게"),
    ("richTextSizeHuge", "Huge", "特大", "特大", "아주 크게"),
    ...
    ("assistantRefineQuestion(details: String)", "Understood — I haven’t saved anything yet. What would you like to change or add? (My proposal was: {details})", "好的，我还没有保存任何内容。你想调整或补充哪些部分？（我原本的提议：{details}）", "承知しました。まだ何も保存していません。どの点を変更・追加したいですか？（私の提案：{details}）", "알겠습니다. 아직 아무것도 저장하지 않았습니다. 무엇을 바꾸거나 추가하고 싶으세요? (제 제안: {details})"),
    ("backupNowFailed", "Backup failed", "备份失败", "バックアップ失敗", "백업 실패"),
    ("backupNowSucceeded", "Backup successful", "备份成功", "バックアップ成功", "백업 성공"),
    ("cloudAutoBackupSub", "Each automatic backup is uploaded to this folder as well as kept locally.", "每次自动备份除保存在本地外，同步上传到该目录。", "自動バックアップをローカル保存に加えてこのフォルダーへも。", "자동 백업을 로컬 저장과 함께 이 폴더로도 업로드합니다."),
    ("cloudAutoBackupTitle", "Also upload automatic backups", "同时上传自动备份", "自動バックアップもアップロード", "자동 백업도 업로드"),
    ("cloudBackupNow", "Back up to cloud now", "立即备份到云盘", "今すぐクラウドへバックアップ", "지금 클라우드로 백업"),
    ("cloudBackupNowDone", "Copied to the cloud.", "已复制到云端。", "クラウドへコピーしました。", "클라우드로 복사했습니다."),
    ("cloudBackupNowFailed", "Cloud upload failed — ", "上传云端失败——", "クラウドへのアップロード失敗 — ", "클라우드 업로드 실패 — "),
    ("cloudCustomProvider", "Custom", "自定义", "カスタム", "사용자 지정"),
    ("cloudEnableSub", "Choose this, then fill in your provider's details below.", "开启后，请在下方填写服务商的连接信息。", "有効にしたら、下にプロバイダーの情報を入力してください。", "켠 뒤 아래에 서비스 정보를 입력하세요."),
    ("cloudEnableTitle", "Enable cloud storage", "启用云存储", "クラウドストレージを有効化", "클라우드 저장소 사용"),
    ("cloudFolderLabel", "Folder (created automatically)", "目录（会自动创建）", "フォルダー（自動作成）", "폴더(자동 생성)"),
    ("cloudNeedsConfig", "Fill in the address, username and password first.", "请先填写地址、用户名和密码。", "先に URL・ユーザー名・パスワードを入力してください。", "먼저 주소·사용자 이름·비밀번호를 입력하세요."),
    ("cloudPasswordLabel", "Password / app password", "密码 / 应用密码", "パスワード / アプリパスワード", "비밀번호 / 앱 비밀번호"),
    ("cloudPasswordPrompt", "This backup needs its password:", "该备份需要密码：", "このバックアップにはパスワードが必要です：", "이 백업에는 비밀번호가 필요합니다:"),
    ("cloudProviderTitle", "Provider", "服务商", "プロバイダー", "제공자"),
    ("cloudRestore", "Restore from cloud…", "从云端恢复…", "クラウドから復元…", "클라우드에서 복원…"),
    ("cloudRestoreConfirm(name: String)", "Restore \"{name}\"? This adds or updates from the backup's contents.", "恢复「{name}」？将按备份内容新增或更新。", "「{name}」を復元しますか？バックアップの内容に合わせて追加・更新されます。", "\"{name}\"을(를) 복원할까요? 백업 내용에 따라 추가·업데이트됩니다."),
    ("cloudRestoreDone", "Restored from the cloud.", "已从云端恢复。", "クラウドから復元しました。", "클라우드에서 복원했습니다."),
    ("cloudRestoreEmpty", "No backups found in the cloud folder.", "云端目录里没有找到备份。", "クラウドフォルダーにバックアップがありません。", "클라우드 폴더에 백업이 없습니다."),
    ("cloudRestorePick", "Choose a backup to restore", "选择要恢复的备份", "復元するバックアップを選択", "복원할 백업 선택"),
    ("cloudSub", "WebDAV backups & restore", "WebDAV 备份与恢复", "WebDAV バックアップと復元", "WebDAV 백업 및 복원"),
    ("cloudTestButton", "Test connection", "测试连接", "接続テスト", "연결 테스트"),
    ("cloudTestRunning", "Testing…", "正在测试…", "テスト中…", "테스트 중…"),
    ("cloudTitle", "Cloud storage", "云存储", "クラウドストレージ", "클라우드 저장소"),
    ("cloudUrlLabel", "Server address (WebDAV URL)", "服务器地址（WebDAV 地址）", "サーバー URL（WebDAV）", "서버 주소(WebDAV URL)"),
    ("cloudUserLabel", "Username", "用户名", "ユーザー名", "사용자 이름"),
    ("lockRetryIn(remaining: String)", "Locked for a moment - try again in {remaining}", "暂时锁定，{remaining} 后再试", "一時ロック中です。{remaining} 後に再試行してください", "잠시 잠금 상태입니다. {remaining} 후 다시 시도하세요"),
    ("notebookA11yAddItems", "Add selected items to a notebook", "把选中的条目加入笔记本", "選択した項目をノートブックに追加", "선택한 항목을 노트북에 추가"),
    ("notebookA11yOpen", "Open notebook", "打开笔记本", "ノートブックを開く", "노트북 열기"),
    ("notebookA11yRemove", "Remove from notebook", "从笔记本移除", "ノートブックから削除", "노트북에서 제거"),
    ("notebookAdd", "Add", "加入", "追加", "추가"),
    ("notebookAddTo", "Add to notebook", "加入笔记本", "ノートブックに追加", "노트북에 추가"),
    ("notebookAddedToast", "Added to notebook", "已加入笔记本", "ノートブックに追加しました", "노트북에 추가했습니다"),
    ("notebookAlreadyInToast", "Already in this notebook", "已在该笔记本中", "このノートブックに追加済みです", "이미 이 노트북에 있습니다"),
    ("notebookCreate", "Create", "创建", "作成", "만들기"),
    ("notebookCreateAndAdd", "Create & add", "创建并加入", "作成して追加", "만들고 추가"),
    ("notebookDeleteBody(title: String)", "“{title}” and its item list will be deleted. The notes and tasks inside it are not affected.", "“{title}”及其条目列表将被删除，里面的笔记和任务不受影响。", "「{title}」とその項目一覧を削除します。中のノートとタスクには影響しません。", "“{title}”과(와) 그 항목 목록이 삭제됩니다. 안에 있는 노트와 할 일은 영향을 받지 않습니다."),
    ...
    ("notebookNewPrompt", "New notebook…", "新建笔记本…", "新しいノートブック…", "새 노트북…"),
    ("notebookPickEmpty", "No notebooks yet. Create one to continue.", "还没有笔记本，先创建一个吧。", "ノートブックがありません。先に作成してください。", "아직 노트북이 없습니다. 먼저 만드세요."),
    ("notebookPickTitle", "Add to notebook", "加入笔记本", "ノートブックに追加", "노트북에 추가"),
    ("notebookRemoveItem", "Remove from notebook", "从笔记本移除", "ノートブックから削除", "노트북에서 제거"),
    ("notebookRemovedToast", "Removed from notebook", "已从笔记本移除", "ノートブックから削除しました", "노트북에서 제거했습니다"),
    ("notebookRename", "Rename notebook", "重命名笔记本", "ノートブック名を変更", "노트북 이름 바꾸기"),
    ("notebookRenameAction", "Rename", "重命名", "変更", "바꾸기"),
    ("notebookRenamedToast", "Notebook renamed", "笔记本已重命名", "ノートブック名を変更しました", "노트북 이름이 변경되었습니다"),
    ("notebookUntitled", "Untitled notebook", "未命名笔记本", "無題のノートブック", "제목 없는 노트북"),
    ("paletteAlpine", "Alpine", "高山", "アルパイン", "알파인"),
    ("paletteAmethyst", "Amethyst", "紫晶", "アメジスト", "자수정"),
    ("paletteCherry", "Cherry", "樱桃", "チェリー", "체리"),
    ("paletteConfetti", "Confetti", "彩纸", "コンフェッティ", "컨페티"),
    ("paletteEggshell", "Eggshell", "蛋壳", "エッグシェル", "에그쉘"),
    ("paletteGrapevine", "Grapevine", "藤蔓", "グレープバイン", "그레이프바인"),
    ("paletteGroupAqua", "Aqua", "水色", "アクア", "아쿠아"),
    ("paletteGroupBlossom", "Blossom", "花季", "花盛り", "꽃"),
    ("paletteGroupDawn", "Dawn", "晨曦", "夜明け", "새벽"),
    ("paletteGroupEarth", "Earth", "大地", "大地", "대지"),
    ("paletteGroupEvergreen", "Evergreen", "常青", "常緑", "상록"),
    ("paletteGroupSunfire", "Sunfire", "焰光", "サンファイア", "선파이어"),
    ("paletteGroupVelvet", "Velvet", "丝绒", "ベルベット", "벨벳"),
    ("paletteGroupVivid", "Vivid", "明艳", "ヴィヴィッド", "비비드"),
    ("paletteMisty", "Misty", "薄雾", "ミスティ", "미스티"),
    ...
    ("paletteTangerine", "Tangerine", "柑橘", "タンジェリン", "탄제린"),
    ("paletteTobacco", "Tobacco", "烟草", "タバコ", "토바코"),
    ("paletteTwilight", "Twilight", "暮色", "トワイライト", "트와일라잇"),
    ("screenNotebooks", "Notebooks", "笔记本", "ノートブック", "노트북"),
    ("themeMonetAbyss", "Abyss", "深渊", "アビス", "어비스"),
    ("themeMonetAbyssDesc", "A deep teal abyss", "深邃的青绿深渊", "深いティールのアビス", "깊은 틸 어비스"),
    ("themeMonetBlueberry", "Blueberry", "蓝莓", "ブルーベリー", "블루베리"),
    ("themeMonetBlueberryDesc", "A rich blueberry", "浓郁的蓝莓色", "濃いブルーベリー", "진한 블루베리"),
    ("themeMonetBrick", "Brick", "砖红", "ブリック", "브릭"),
    ("themeMonetBrickDesc", "A deep brick-red", "深邃的砖红", "深いレンガ色", "깊은 브릭 레드"),
    ("themeMonetClay", "Clay", "陶土", "クレイ", "클레이"),
    ("themeMonetClayDesc", "A warm burnt umber", "温暖的焦赭色", "暖かなバーントアンバー", "따뜻한 번트 엄버"),
    ("themeMonetCoral", "Coral", "珊瑚", "コーラル", "코랄"),
    ("themeMonetCoralDesc", "A warm coral", "温暖的珊瑚色", "あたたかいコーラル", "따뜻한 코랄"),
    ("themeMonetDeepMoss", "Deep Moss", "深苔", "ディープモス", "딥 모스"),
    ("themeMonetDeepMossDesc", "A deep mossy green", "深邃的苔绿", "深いコケグリーン", "깊은 이끼 그린"),
    ("themeMonetFir", "Fir", "冷杉", "モミ", "전나무"),
    ("themeMonetFirDesc", "A deep fir green", "深邃的冷杉绿", "深いモミグリーン", "깊은 전나무 그린"),
    ("themeMonetFrost", "Frost", "霜", "フロスト", "프로스트"),
    ("themeMonetFrostDesc", "A cool frost blue", "清冷的霜蓝", "涼しげなフロストブルー", "시원한 프로스트 블루"),
    ("themeMonetHoney", "Honey", "蜂蜜", "ハニー", "허니"),
    ("themeMonetHoneyDesc", "A warm golden cream", "温暖的金奶油色", "やわらかなゴールドクリーム", "따뜻한 골든 크림"),
    ("themeMonetHoneydew", "Honeydew", "蜜瓜", "ハニーデュー", "허니듀"),
    ("themeMonetHoneydewDesc", "A fresh melon green", "清新的蜜瓜绿", "さわやかなメロングリーン", "상쾌한 멜론 그린"),
    ("themeMonetLinen", "Linen", "亚麻", "リネン", "리넨"),
    ("themeMonetLinenDesc", "A pale linen", "淡雅的亚麻色", "淡いリネン", "연한 리넨"),
    ("themeMonetMint", "Mint", "薄荷", "ミント", "민트"),
    ("themeMonetMintDesc", "A soft mint", "柔和的薄荷绿", "やわらかなミント", "부드러운 민트"),
    ("themeMonetObsidian", "Obsidian", "黑曜石", "オブシディアン", "옵시디언"),
    ("themeMonetObsidianDesc", "A deep blue-grey obsidian", "深邃的蓝灰黑曜石", "深い青灰オブシディアン", "깊은 블루그레이 옵시디언"),
    ("themeMonetOrchid", "Orchid", "兰花", "オーキッド", "오키드"),
    ("themeMonetOrchidDesc", "A soft mauve", "柔和的紫粉色", "やわらかなモーヴ", "부드러운 모브"),
    ("themeMonetOrchidNight", "Orchid Night", "兰花之夜", "オーキッドナイト", "오키드 나이트"),
    ("themeMonetOrchidNightDesc", "A deep mauve", "深邃的紫褐", "深いモーヴ", "깊은 모브"),
    ("themeMonetPeach", "Peach", "蜜桃", "ピーチ", "피치"),
    ("themeMonetPeachDesc", "A soft apricot", "柔和的杏色", "やわらかなアプリコット", "부드러운 살구색"),
    ("themeMonetPeriwinkle", "Periwinkle", "长春花", "ペリウィンクル", "페리윙클"),
    ("themeMonetPeriwinkleDesc", "A gentle blue-violet", "柔和的蓝紫色", "やさしい青紫", "은은한 블루 바이올렛"),
    ("themeMonetSnowdrop", "Snowdrop", "雪滴", "スノードロップ", "스노드롭"),
    ("themeMonetSnowdropDesc", "An icy near-white", "冰冷的近白色", "氷のような白", "차가운 아이스 화이트"),
    ("themeMonetUmber", "Umber", "赭色", "アンバー", "엄버"),
    ("themeMonetUmberDesc", "A deep burnt umber", "深邃的焦赭色", "深いバーントアンバー", "깊은 번트 엄버"),
    ("themeMonetWalnut", "Walnut", "胡桃", "ウォールナット", "호두"),
    ("themeMonetWalnutDesc", "A rich walnut brown", "浓郁的胡桃棕", "豊かなウォールナットブラウン", "풍부한 호두 브라운"),
    ("tplAuthoringTitle", "Designing a new template", "正在设计新模板", "新テンプレートを作成中", "새 템플릿 만들기"),
    ("tplContinueDraft", "Continue unfinished template", "继续未完成的模板", "未完成のテンプレートを続ける", "완성하지 못한 템플릿 계속하기"),
    ("tplCreateCustom", "New template", "新建模板", "新しいテンプレート", "새 템플릿"),
    ("tplDeleteBody(name: String)", "\"{name}\" will be removed from the template list.", "「{name}」将从模板列表中移除。", "「{name}」がテンプレート一覧から削除されます。", "\"{name}\"이(가) 템플릿 목록에서 삭제됩니다."),
    ("tplDeleteTitle", "Delete this template?", "删除这个模板？", "このテンプレートを削除しますか？", "이 템플릿을 삭제할까요?"),
    ("tplDiscard", "Discard", "放弃", "破棄", "버리기"),
    ("tplDraftKeptToast", "Unfinished template kept — offered again next time.", "未完成的模板已保留——下次会自动提示继续。", "未完成のテンプレートを保持しました。次回またお知らせします。", "완성하지 못한 템플릿을 보관했습니다. 다음에 다시 알려드립니다."),
    ("tplMenuHint", "Edit or delete this template", "编辑或删除这个模板", "このテンプレートを編集または削除", "이 템플릿을 편집하거나 삭제"),
    ("tplNeedsTitle", "Give the template a title first.", "请先为模板填写标题。", "先にテンプレートのタイトルを入力してください。", "먼저 템플릿 제목을 입력하세요."),
    ("tplRestoreAll", "Restore all", "全部恢复", "すべて復元", "모두 복원"),
    ("tplRestoreBuiltins", "Restore built-in templates", "恢复内置模板", "組み込みを復元", "기본 제공 복원"),
    ("tplSaveAsTemplate", "Save as template", "存为模板", "テンプレートとして保存", "템플릿으로 저장"),
    ("tplSavedToast", "Template saved", "模板已保存", "テンプレートを保存しました", "템플릿이 저장되었습니다"),

    ("agentTraceTitle", "Agent steps", "执行步骤", "エージェントの手順", "에이전트 단계"),
    ("agentTraceStepCount(count: Int)", "{count} steps", "{count} 个步骤", "{count} ステップ", "{count}단계"),
    ("agentTraceThinking", "Thinking", "思考过程", "思考プロセス", "사고 과정"),
    ("agentTraceRunning", "Running", "运行中", "実行中", "실행 중"),
    ("agentTraceFailed", "Failed", "失败", "失敗", "실패"),
    ("agentTraceCancelled", "Cancelled", "已取消", "キャンセル済み", "취소됨"),
    ("agentTraceRefining", "You asked for changes", "你要求修改", "変更を依頼しました", "변경을 요청함"),
    ("agentTraceError", "Something went wrong", "出错了", "問題が発生しました", "문제가 발생했습니다"),
    ("agentTraceRetrying(attempt: Int)", "Retrying (attempt {attempt})", "重试中（第 {attempt} 次）", "再試行中（{attempt} 回目）", "재시도 중({attempt}번째)"),
    ("agentTracePlanning", "Planning the reply", "规划回复", "返信を組み立て中", "답변 구성 중"),
    ("agentTraceUnknownTool", "No such tool", "没有这个工具", "そのツールはありません", "그런 도구가 없습니다"),
    ("agentStepCreateNote", "Creating note", "创建笔记", "メモを作成", "노트 만들기"),
    ("agentStepListNotes", "Listing notes", "列出笔记", "メモを一覧", "노트 나열"),
    ("agentStepReadNote", "Reading note", "读取笔记", "メモを読み込み", "노트 읽기"),
    ("agentStepUpdateNote", "Editing note", "编辑笔记", "メモを編集", "노트 편집"),
    ("agentStepDelete", "Deleting", "删除", "削除", "삭제"),
    ("agentStepPin", "Pinning", "置顶", "ピン留め", "고정"),
    ("agentStepUnpin", "Unpinning", "取消置顶", "ピン留めを解除", "고정 해제"),
    ("agentStepArchive", "Archiving", "归档", "アーカイブ", "보관"),
    ("agentStepUnarchive", "Unarchiving", "取消归档", "アーカイブを解除", "보관 해제"),
    ("agentStepNoteColor", "Recolouring note", "设置笔记颜色", "メモの色を変更", "노트 색상 변경"),
    ("agentStepNoteChecklist", "Updating note checklist", "更新笔记清单", "メモのチェックリストを更新", "노트 체크리스트 업데이트"),
    ("agentStepNoteLayout", "Changing note layout", "转换笔记格式", "メモの形式を変更", "노트 형식 변경"),
    ("agentStepNoteAttachment", "Updating note attachment", "更新笔记附件", "メモの添付を更新", "노트 첨부 업데이트"),
    ("agentStepNoteVersions", "Reading note versions", "读取笔记版本", "メモの履歴を読み込み", "노트 버전 읽기"),
    ("agentStepCreateTask", "Creating task", "创建任务", "タスクを作成", "할 일 만들기"),
    ("agentStepListTasks", "Listing tasks", "列出任务", "タスクを一覧", "할 일 나열"),
    ("agentStepReadTask", "Reading task", "读取任务", "タスクを読み込み", "할 일 읽기"),
    ("agentStepCompleteTask", "Completing task", "完成任务", "タスクを完了", "할 일 완료"),
    ("agentStepReopenTask", "Reopening task", "重新打开任务", "タスクを再開", "할 일 다시 열기"),
    ("agentStepUpdateTask", "Editing task", "编辑任务", "タスクを編集", "할 일 편집"),
    ("agentStepTaskPriority", "Setting priority", "设置优先级", "優先度を設定", "우선순위 설정"),
    ("agentStepTaskDue", "Setting due date", "设置截止时间", "期限を設定", "마감일 설정"),
    ("agentStepSubtask", "Updating subtask", "更新子任务", "サブタスクを更新", "하위 작업 업데이트"),
    ("agentStepTaskAttachment", "Updating task attachment", "更新任务附件", "タスクの添付を更新", "할 일 첨부 업데이트"),
    ("agentStepListDrafts", "Listing drafts", "列出草稿", "下書きを一覧", "임시 보관 나열"),
    ("agentStepListTrash", "Listing trash", "列出回收站", "ゴミ箱を一覧", "휴지통 나열"),
    ("agentStepRestoreTrash", "Restoring from trash", "从回收站恢复", "ゴミ箱から復元", "휴지통에서 복원"),
    ("agentStepReadAttachment", "Reading attachment", "读取附件", "添付を読み込み", "첨부 읽기"),
    ("agentStepSearch", "Searching", "搜索", "検索", "검색"),
    ("agentStepRecallNotes", "Recalling notes", "回忆笔记", "メモを想起", "노트 기억"),
    ("agentStepWebSearch", "Searching the web", "联网搜索", "ウェブ検索", "웹 검색"),
    ("agentStepFormatText", "Formatting text", "设置文字格式", "文字の書式を設定", "텍스트 서식 지정"),
    ("agentStepAppendText", "Adding text", "追加文字", "文章を追加", "글 추가"),
    ("agentStepHideItem", "Hidden area", "隐藏区", "隠しエリア", "숨김 영역"),
    ("agentStepMoveItem", "Moving it", "移动位置", "移動", "이동"),
    ("agentStepNotebookCover", "Notebook cover", "笔记本封面", "ノートブックの表紙", "노트북 표지"),
    ("agentStepNotebookOrder", "Notebook order", "笔记本排序", "ノートブックの並び", "노트북 순서"),
    ("agentStepNotebookTrash", "Notebook trash", "笔记本回收站", "ノートブックのゴミ箱", "노트북 휴지통"),
    ("formatAutoDetect", "Auto-detect", "自动检测", "自動判別", "자동 감지"),
    ...
    ("aboutCheckFailed", "Failed to check updates", "检查更新失败", "更新の確認に失敗しました", "업데이트 확인 실패"),
    ("aboutCheckUpdate", "Check for updates", "检查更新", "更新を確認", "업데이트 확인"),
    ("aboutContact", "Contact", "联系方式", "連絡先", "연락처"),
    ("aboutCopyright", "Copyright", "版权所有", "著作権", "저작권"),
    ("aboutDeveloper", "Developer", "开发者", "開発者", "개발자"),
    ("aboutGithub", "GitHub", "GitHub", "GitHub", "GitHub"),
    ("aboutLicense", "Open Source Licenses", "开源许可", "オープンソースライセンス", "오픈소스 라이선스"),
    ("aboutLicenseDetail", "This software uses code from Kotlin, Compose Multiplatform, SQLCipher, llama.cpp, and others under Apache-2.0, MIT, BSD and similar permissive licenses.", "本软件使用了 Kotlin、Compose Multiplatform、SQLCipher、llama.cpp 等开源项目的代码，遵循 Apache-2.0、MIT、BSD 等宽松许可协议。", "本ソフトウェアはKotlin、Compose Multiplatform、SQLCipher、llama.cppなどのオープンソースコードをApache-2.0、MIT、BSD等の寛容なライセンスのもとで使用しています。", "이 소프트웨어는 Kotlin, Compose Multiplatform, SQLCipher, llama.cpp 등의 오픈소스 코드를 Apache-2.0, MIT, BSD 등 허용적 라이선스 하에 사용합니다."),
    ("aboutLicenseFull", "See docs/THIRD-PARTY-NOTICES.md for full details.", "详见 docs/THIRD-PARTY-NOTICES.md。", "詳細は docs/THIRD-PARTY-NOTICES.md をご覧ください。", "자세한 내용은 docs/THIRD-PARTY-NOTICES.md를 참조하세요."),
    ("aboutNewVersion", "New version available:", "有新版本可用：", "新しいバージョンがあります：", "새 버전 사용 가능:"),
    ("aboutRights", "All rights reserved.", "保留所有权利。", "無断転載を禁じます。", "모든 권리 보유."),
    ("aboutUpToDate", "You have the latest version:", "已是最新版本：", "最新バージョンです：", "최신 버전입니다:"),
    ("aboutVersion", "Version", "版本", "バージョン", "버전"),
    ("advancedEmptyHint", "No advanced options are available on this platform yet.", "此平台暂无高级选项。", "このプラットフォームで利用できる詳細設定はまだありません。", "이 플랫폼에서 아직 사용할 수 있는 고급 옵션이 없습니다."),
    ("advancedTitle", "Advanced", "高级", "詳細設定", "고급"),
    ("settingsAboutSub", "Version, licenses, updates", "版本、许可、更新", "バージョン、ライセンス、更新", "버전, 라이선스, 업데이트"),
    ("settingsAboutTitle", "About", "关于", "情報", "정보"),
    ("settingsAdvancedSub", "Privileged operations and extra tools", "特权操作与额外工具", "特権操作と追加ツール", "특권 작업과 추가 도구"),
    ("settingsAdvancedTitle", "Advanced", "高级", "詳細設定", "고급"),
    ("shizukuDesc", "Grant elevated privileges via Shizuku service", "通过 Shizuku 服务授予特权限权", "Shizukuサービスを介して特権を付与", "Shizuku 서비스를 통해 특권 부여"),
    ("shizukuNotInstalled", "Shizuku isn't installed on this device", "此设备未安装 Shizuku", "この端末にShizukuがインストールされていません", "이 기기에 Shizuku가 설치되어 있지 않습니다"),
    ("shizukuNotReady", "Shizuku not available", "Shizuku 不可用", "Shizuku利用不可", "Shizuku 사용 불가"),
    ("shizukuPair", "Pair Shizuku", "配对 Shizuku", "Shizukuをペアリング", "Shizuku 페어링"),
    ("shizukuReady", "Shizuku is ready", "Shizuku 已就绪", "Shizuku利用可能", "Shizuku 사용 가능"),
    ("shizukuTitle", "Shizuku", "Shizuku", "Shizuku", "Shizuku"),

    ("aboutTagline", "Notes, tasks, and an assistant that keeps your business to itself.", "笔记、任务，以及一位守口如瓶的助手。", "メモ、タスク、そして口の堅いアシスタント。", "노트, 할 일, 그리고 입이 무거운 어시스턴트."),
    ("aboutHomepage", "Homepage", "官方主页", "公式サイト", "공식 홈페이지"),
    ("aboutLicensesOpen", "third-party licences and their full texts", "第三方许可与许可全文", "サードパーティライセンスと全文", "서드파티 라이선스 및 전문"),
    ("licencesTitle", "Open-source licences", "开源许可", "オープンソースライセンス", "오픈소스 라이선스"),
    ("licencesIntro", "Lucent is built on the open-source projects below. Each entry links to its licence in full.", "Lucent 建立在下列开源项目之上。每一项都可打开许可全文。", "Lucent は以下のオープンソースプロジェクトの上に成り立っています。各項目からライセンス全文を開けます。", "Lucent는 아래 오픈소스 프로젝트 위에 세워졌습니다. 각 항목에서 라이선스 전문을 열 수 있습니다."),
    ("licencesFullText", "Full licence text", "许可全文", "ライセンス全文", "라이선스 전문"),
    ("licencesAllNotices", "Complete third-party notices", "完整第三方声明", "サードパーティ通知の全文", "서드파티 고지 전문"),
    ("shizukuEnableDesc", "Let Lucent run privileged operations through Shizuku. Off unless you ask for it.", "允许 Lucent 通过 Shizuku 执行特权操作。默认关闭，需要时再开启。", "Shizuku を通じて Lucent に特権操作を許可します。必要なときだけオンにしてください。", "Shizuku를 통해 Lucent가 특권 작업을 실행하도록 허용합니다. 필요할 때만 켜세요."),
    ("shizukuStatusOff", "Off", "已关闭", "オフ", "꺼짐"),
    ("shizukuStatusNoApp", "Shizuku is not installed on this device", "此设备未安装 Shizuku", "この端末に Shizuku がインストールされていません", "이 기기에 Shizuku가 설치되어 있지 않습니다"),
    ("shizukuStatusNoService", "Shizuku is installed, but its service is not running", "Shizuku 已安装，但服务未运行", "Shizuku はインストール済みですが、サービスが起動していません", "Shizuku가 설치되어 있지만 서비스가 실행 중이 아닙니다"),
    ("shizukuStatusNoPermission", "Shizuku is running \u2014 permission not granted yet", "Shizuku 正在运行——尚未授予权限", "Shizuku は動作中です\u2014まだ権限がありません", "Shizuku가 실행 중입니다\u2014아직 권한이 없습니다"),
    ("shizukuActionInstall", "Get Shizuku", "获取 Shizuku", "Shizuku を入手", "Shizuku 받기"),
    ("shizukuActionOpen", "Open Shizuku", "打开 Shizuku", "Shizuku を開く", "Shizuku 열기"),
    ("shizukuActionGrant", "Grant permission", "授予权限", "権限を許可", "권한 허용"),
    ("advancedElevateTitle", "Administrator privileges", "管理员权限", "管理者権限", "관리자 권한"),
    ("advancedElevateDesc", "Run Lucent's privileged operations as the system administrator. The system will ask for consent.", "以系统管理员身份执行 Lucent 的特权操作。系统会请求你的同意。", "Lucent の特権操作をシステム管理者として実行します。システムが確認を求めます。", "Lucent의 특권 작업을 시스템 관리자로 실행합니다. 시스템이 동의를 요청합니다."),
    ("advancedElevatedStatus", "Lucent is running with administrator privileges", "Lucent 正以管理员权限运行", "Lucent は管理者権限で実行中です", "Lucent가 관리자 권한으로 실행 중입니다"),
    ...
    ("updateLater", "Later", "稍后", "あとで", "나중에"),
    ("updateChecking", "Checking\u2026", "正在检查…", "確認中…", "확인 중…"),
    ...
    ("updateNeedsFolder", "Choose a folder for the update download. The backup folder from Settings - Data is used by default.", "请选择更新安装包的下载文件夹。默认使用「设置 - 数据」里的备份文件夹。", "更新のダウンロード先フォルダーを選んでください。既定では「設定 - データ」のバックアップフォルダーを使います。", "업데이트를 내려받을 폴더를 선택하세요. 기본값은 설정 - 데이터의 백업 폴더입니다."),
    ("updateChooseFolder", "Choose folder", "选择文件夹", "フォルダーを選択", "폴더 선택"),
    ("updateHide", "Hide", "隐藏", "隠す", "숨기기"),
    ("updateBackgroundHint", "The download continues in the background; its progress and Cancel are in the notification.", "下载会在后台继续；进度和取消都在通知栏里。", "ダウンロードはバックグラウンドで続きます。進捗と中止は通知から操作できます。", "내려받기는 백그라운드에서 계속되며, 진행 상황과 취소는 알림에서 확인할 수 있습니다."),
    ("updateDownloadCancelled", "Download cancelled. The partial file was removed.", "下载已取消，未完成的文件已删除。", "ダウンロードを中止しました。途中のファイルは削除しました。", "내려받기를 취소했고, 일부 받은 파일을 삭제했습니다."),
    ("updateChannelName", "Update downloads", "更新下载", "更新のダウンロード", "업데이트 내려받기"),
    ("updateChannelDesc", "Progress while a Lucent update downloads.", "Lucent 更新下载时的进度提示。", "Lucent の更新をダウンロードしている間の進捗です。", "Lucent 업데이트를 내려받는 동안의 진행 상황입니다."),
    ("updateNotifTitle", "Lucent update", "Lucent 更新", "Lucent の更新", "Lucent 업데이트"),
    ("updateNotifPreparing", "Preparing the download\u2026", "正在准备下载…", "ダウンロードの準備中…", "내려받기를 준비하는 중…"),
    ("updateNotifDownloading(percent: Int)", "Downloading - {percent}%", "正在下载 - {percent}%", "ダウンロード中 - {percent}%", "내려받는 중 - {percent}%"),
    ("updateNotifCancel", "Cancel", "取消", "キャンセル", "취소"),
    ("updateNotifReady", "Downloaded and ready to install. Tap to open Lucent.", "已下载，可以安装。点击打开 Lucent。", "ダウンロード完了、インストールできます。タップで Lucent を開きます。", "내려받기 완료, 설치할 수 있습니다. 탭하면 Lucent가 열립니다."),
    ("updateNotifReadyStored", "Downloaded into your backup folder and ready to install. Tap to open Lucent.", "已下载到你的备份文件夹，可以安装。点击打开 Lucent。", "バックアップフォルダーに保存し、インストールできます。タップで Lucent を開きます。", "백업 폴더에 저장했고 설치할 수 있습니다. 탭하면 Lucent가 열립니다."),
    ...
    ("settingsCloudModelTitle", "Cloud model", "云端模型", "クラウドモデル", "클라우드 모델"),
    ("settingsCloudModelSub(active: String)", "Selection, connection, memory · active: {active}", "选择、连接与记忆 · 当前：{active}", "選択・接続・記憶 · 使用中：{active}", "선택·연결·기억 · 사용 중: {active}"),
    ("settingsLocalModelSubOn", "On-device assistant, running now", "本机助手，正在运行", "端末内アシスタント（稼働中）", "기기 내 어시스턴트, 실행 중"),
    ("agentModeTitle", "Agent mode", "Agent 模式", "エージェントモード", "에이전트 모드"),
    ("agentModeSub", "Thinks, uses tools, and shows every step. Off replies fastest.", "会思考、调用工具并展示每一步。关闭后回复最快。", "考え、ツールを使い、各手順を表示します。オフなら最速で返します。", "생각하고 도구를 쓰며 모든 단계를 보여 줍니다. 끄면 가장 빠릅니다."),
    ("memoryWeakTitle", "Weak · one message at a time", "弱 · 每次只记住一条消息", "弱 · 1メッセージずつ", "약 · 한 번에 한 메시지"),
    ("memoryStrongTitle", "Strong · the whole conversation", "强 · 记住整段对话", "強 · 会話の全体", "강 · 대화 전체"),
    ("typingHapticsSub", "A light tick as the reply types itself out", "文字逐字出现时轻微震动", "返信が打ち出されるときに軽く振動します", "답변이 타이핑될 때 가볍게 울립니다"),
    ("webSearchSub", "Let the assistant look things up for answers that must be current", "允许助手联网查找需要最新信息的答案", "最新の情報が必要な答えのために検索を許します", "최신 정보가 필요한 답을 위해 검색을 허용합니다"),
    ("assistantConfirmToolsSub", "Ask before changing a note or a task", "改动笔记或任务前先征求确认", "ノートやタスクを変更する前に確認します", "노트나 할 일을 바꾸기 전에 확인합니다"),
    ("smallModelModeSub", "Trim the prompts for weaker or smaller models", "为能力较弱或体积更小的模型精简提示词", "小さい・弱いモデル向けにプロンプトを削ります", "작거나 약한 모델을 위해 프롬프트를 줄입니다"),
    ("aboutPrivacy", "Privacy policy", "隐私政策", "プライバシーポリシー", "개인정보 처리방침"),
    ("aboutPrivacyOpen", "how Lucent handles your data", "查看 Lucent 如何处理你的数据", "Lucent がデータをどう扱うか", "Lucent가 데이터를 다루는 방식"),
    ("updateNotesOnGitHub", "See what's new on GitHub", "前往 GitHub 查看更新内容", "GitHub で更新内容を見る", "GitHub에서 업데이트 내용 보기"),
    ("a11ySearchNotebooks", "Search notebooks", "搜索笔记本", "ノートブックを検索", "노트북 검색"),
    ("notebookNoMatch", "No notebooks match your search.", "没有匹配的笔记本。", "検索に一致するノートブックはありません。", "검색과 일치하는 노트북이 없습니다."),
    ("notebookTrashBody(title: String)", "Move the notebook \"{title}\" to the trash? The notes and tasks inside it are kept.", "把笔记本“{title}”移入回收站？里面的笔记和任务都会保留。", "ノートブック「{title}」をゴミ箱に移動しますか？中のノートとタスクは残ります。", "노트북 \"{title}\"을(를) 휴지통으로 옮길까요? 안에 있는 노트와 할 일은 그대로 남습니다."),
    ("notebookTrashEmpty", "The notebook trash is empty.", "笔记本回收站是空的。", "ノートブックのゴミ箱は空です。", "노트북 휴지통이 비어 있습니다."),
    ("notebookEmptyTrashBody", "Every notebook in the trash will be deleted for good. The notes and tasks inside them are kept.", "回收站里的笔记本都会被彻底删除，里面的笔记和任务都会保留。", "ゴミ箱にあるノートブックはすべて完全に削除されます。中のノートとタスクは残ります。", "휴지통의 모든 노트북이 완전히 삭제됩니다. 안에 있는 노트와 할 일은 그대로 남습니다."),
    ("notebookCoverTitle", "Cover", "封面", "表紙", "표지"),
    ("notebookCoverA11y(name: String)", "{name} cover", "{name} 的封面", "{name} の表紙", "{name} 표지"),
    ("coverCrimson", "Crimson", "绯红", "クリムゾン", "크림슨"),
    ("coverOcean", "Ocean", "海洋蓝", "オーシャン", "오션"),
    ("coverForest", "Forest", "森林绿", "フォレスト", "포레스트"),
    ("coverAmber", "Amber", "琥珀", "アンバー", "앰버"),
    ("coverPlum", "Plum", "李子紫", "プラム", "플럼"),
    ("ccTrashNotebook(title: String)", "Move the notebook \"{title}\" to the trash", "把笔记本“{title}”移入回收站", "ノートブック「{title}」をゴミ箱へ移動", "노트북 \"{title}\"을(를) 휴지통으로 이동"),
    ("ccRestoreNotebook(title: String)", "Restore the notebook \"{title}\" from the trash", "从回收站恢复笔记本“{title}”", "ゴミ箱からノートブック「{title}」を復元", "휴지통에서 노트북 \"{title}\" 복원"),
    ("ccSetNotebookCover(notebook: String, cover: String)", "Give the notebook \"{notebook}\" a {cover} cover", "把笔记本“{notebook}”的封面换成{cover}", "ノートブック「{notebook}」の表紙を{cover}に", "노트북 \"{notebook}\"의 표지를 {cover}(으)로"),
    ("ccMoveNotebook(notebook: String, position: String)", "Move the notebook \"{notebook}\" ({position})", "移动笔记本“{notebook}”（{position}）", "ノートブック「{notebook}」を移動（{position}）", "노트북 \"{notebook}\" 이동({position})"),
    ("ccFormatNote(find: String, title: String)", "Format \"{find}\" in the note \"{title}\"", "为笔记“{title}”中的“{find}”设置格式", "メモ「{title}」の「{find}」を書式設定", "노트 \"{title}\"의 \"{find}\" 서식 지정"),
    ("ccFormatTask(find: String, title: String)", "Format \"{find}\" in the task \"{title}\"", "为任务“{title}”中的“{find}”设置格式", "タスク「{title}」の「{find}」を書式設定", "할 일 \"{title}\"의 \"{find}\" 서식 지정"),
    ("ccAppendNote(title: String)", "Add text to the end of the note \"{title}\"", "在笔记“{title}”末尾追加文字", "メモ「{title}」の末尾に文章を追加", "노트 \"{title}\" 끝에 글 추가"),
    ("ccAppendTask(title: String)", "Add text to the end of the task \"{title}\"", "在任务“{title}”末尾追加文字", "タスク「{title}」の末尾に文章を追加", "할 일 \"{title}\" 끝에 글 추가"),
    ("ccSetNoteFormat(title: String, format: String)", "Show the note \"{title}\" as {format}", "把笔记“{title}”显示为{format}", "メモ「{title}」を{format}で表示", "노트 \"{title}\"을(를) {format}(으)로 표시"),
    ("ccSetTaskFormat(title: String, format: String)", "Show the task \"{title}\" as {format}", "把任务“{title}”显示为{format}", "タスク「{title}」を{format}で表示", "할 일 \"{title}\"을(를) {format}(으)로 표시"),
    ("ccHideNote(title: String)", "Hide the note \"{title}\"", "隐藏笔记“{title}”", "メモ「{title}」を隠す", "노트 \"{title}\" 숨기기"),
    ("ccShowNote(title: String)", "Bring the note \"{title}\" back", "恢复显示笔记“{title}”", "メモ「{title}」を再表示", "노트 \"{title}\" 다시 표시"),
    ("ccHideTask(title: String)", "Hide the task \"{title}\"", "隐藏任务“{title}”", "タスク「{title}」を隠す", "할 일 \"{title}\" 숨기기"),
    ("ccShowTask(title: String)", "Bring the task \"{title}\" back", "恢复显示任务“{title}”", "タスク「{title}」を再表示", "할 일 \"{title}\" 다시 표시"),
    ("ccMoveNote(title: String, where: String)", "Move the note \"{title}\" {where}", "把笔记“{title}”移到{where}", "メモ「{title}」を{where}へ移動", "노트 \"{title}\"을(를) {where}(으)로 이동"),
    ("ccMoveTask(title: String, where: String)", "Move the task \"{title}\" {where}", "把任务“{title}”移到{where}", "タスク「{title}」を{where}へ移動", "할 일 \"{title}\"을(를) {where}(으)로 이동"),
    ("reasoningTitle", "Reasoning effort", "推理程度", "推論の程度", "추론 수준"),
    ("reasoningDefault", "Provider default", "服务商默认", "プロバイダー既定", "제공자 기본값"),
    ("reasoningDefaultSub", "Leave the provider's own default", "使用服务商自己的默认值", "プロバイダー既定のまま", "제공자 기본값 사용"),
    ("reasoningNone", "Off", "关闭", "オフ", "끔"),
    ("reasoningNoneSub", "No thinking at all", "完全不思考", "まったく考えない", "전혀 생각하지 않음"),
    ("reasoningMinimal", "Minimal", "极低", "最小", "최소"),
    ("reasoningMinimalSub", "The least thinking the model allows", "模型允许的最低思考量", "モデルが許す最小限の思考", "모델이 허용하는 최소한의 사고"),
    ("reasoningLow", "Low", "低", "低", "낮음"),
    ("reasoningLowSub", "A little thinking, quickest answers", "少量思考，回答最快", "少し考えて最速で回答", "조금만 생각하고 가장 빠르게 답변"),
    ("reasoningMedium", "Medium", "中", "中", "보통"),
    ("reasoningMediumSub", "Balanced thinking and speed", "思考与速度平衡", "思考と速度のバランス", "사고와 속도의 균형"),
    ("reasoningHigh", "High", "高", "高", "높음"),
    ("reasoningHighSub", "Thinks hardest, slower answers", "思考最充分，回答较慢", "最も深く考え、回答は遅め", "가장 깊이 생각하고 답변은 느림"),
    ("reasoningXhigh", "Extra high", "极高", "非常に高い", "매우 높음"),
    ("reasoningXhighSub", "More thinking than high, on the models that offer it", "比“高”思考更多，仅部分模型提供", "「高」より深く考える（対応モデルのみ）", "'높음'보다 더 깊이 생각합니다(일부 모델만)"),
    ("reasoningMax", "Maximum", "最高", "最大", "최대"),
    ("reasoningMaxSub", "The most thinking the model can be asked for", "模型允许的最大思考量", "モデルに要求できる最大の思考量", "모델에 요청할 수 있는 최대 사고량"),
    ("agentTraceCacheUsed(tokens: String, percent: Int)", "Prompt cache: {tokens} input tokens reused ({percent}%)", "提示缓存：复用输入 {tokens} token（{percent}%）", "プロンプトキャッシュ：入力 {tokens} トークンを再利用（{percent}%）", "프롬프트 캐시: 입력 {tokens} 토큰 재사용({percent}%)"),
    ("webSearchEngineTitle", "Search engine", "搜索引擎", "検索エンジン", "검색 엔진"),
    ("webSearchEngineSub", "Which engine the assistant looks things up with", "助手用哪个引擎查找资料", "アシスタントが使う検索エンジン", "어시스턴트가 사용할 검색 엔진"),
    ("searchEngineAuto", "Automatic", "自动", "自動", "자동"),
    ("searchEngineBaidu", "Baidu", "百度", "Baidu（百度）", "바이두"),
    ("searchEngineSogou", "Sogou", "搜狗", "Sogou（搜狗）", "서우거우"),
    ("searchEngine360", "360 Search", "360 搜索", "360 検索", "360 검색"),
    ("memoryHighTitle", "High · across conversations", "高 · 跨对话", "高 · 会話をまたぐ", "높음 · 대화 간"),
    ("memoryMediumTitle", "Medium · this conversation", "中 · 本对话", "中 · この会話", "보통 · 이 대화"),
    ("memoryLowTitle", "Low · one message at a time", "低 · 只回复单条消息", "低 · 1メッセージずつ", "낮음 · 한 메시지씩"),
    ("memoryCloudHighSub", "Also remembers your other conversations", "还会记住你的其他对话", "他の会話も覚えます", "다른 대화도 기억합니다"),
    ("memoryCloudMediumSub", "Remembers this whole conversation", "记住当前这段对话", "この会話の全体を覚えます", "이 대화 전체를 기억합니다"),
    ("memoryCloudLowSub", "Each message is answered on its own", "每条消息单独回复，没有记忆", "各メッセージを単独で返します", "각 메시지를 따로 답합니다"),
    ("memoryLocalStrongSub", "Remembers this whole conversation", "记住整段对话", "会話の全体を覚えます", "대화 전체를 기억합니다"),
    ("memoryLocalWeakSub", "Each message is answered on its own", "只回复每一条消息，没有记忆", "各メッセージを単独で返します", "각 메시지를 따로 답합니다"),
    ("settingsCloudModelSubOff", "Off — the local assistant answers", "已关闭——由本地助手作答", "オフ — ローカルアシスタントが応答します", "꺼짐 — 로컬 어시스턴트가 답합니다"),
    ...
    ("harnessGroupFiles", "Files", "文件", "ファイル", "파일"),
    ("harnessGroupTerminal", "Terminal", "终端", "ターミナル", "터미널"),
    ("harnessGroupGit", "Git", "Git", "Git", "Git"),
    ("harnessGroupGithub", "GitHub", "GitHub", "GitHub", "GitHub"),
    ("harnessGroupOffice", "Documents", "文档", "文書", "문서"),
    ("harnessGroupPdf", "PDF", "PDF", "PDF", "PDF"),
    ("harnessGroupBrowser", "Web", "网页", "ウェブ", "웹"),
    ("harnessGroupData", "Data", "数据", "データ", "데이터"),
    ("harnessGroupSandbox", "Sandbox", "沙箱", "サンドボックス", "샌드박스"),
    ("harnessGroupMemory", "Memory", "记忆", "記憶", "기억"),
    ("harnessGroupPlan", "Planning", "计划", "計画", "계획"),
    ("harnessGroupAgent", "Sub-agents", "子助手", "サブエージェント", "하위 에이전트"),
    ("harnessGroupSkills", "Skills", "技能", "スキル", "스킬"),
    ("harnessGroupMcp", "MCP", "MCP", "MCP", "MCP"),
    ("harnessGroupPlugins", "Plugins", "插件", "プラグイン", "플러그인"),
    ("harnessGroupDevice", "Device", "设备", "端末", "기기"),
    ("harnessGroupConnectors", "Connectors", "连接器", "コネクター", "커넥터"),
    ("harnessPermissionRead", "Read", "读取", "読み取り", "읽기"),
    ("harnessPermissionReadSub", "Read files inside the workspace", "读取工作区内的文件", "ワークスペース内のファイルを読む", "작업 공간의 파일 읽기"),
    ("harnessPermissionWrite", "Write", "写入", "書き込み", "쓰기"),
    ("harnessPermissionWriteSub", "Create, edit and move files", "创建、修改和移动文件", "ファイルの作成・編集・移動", "파일 생성, 편집, 이동"),
    ("harnessPermissionDelete", "Delete", "删除", "削除", "삭제"),
    ("harnessPermissionDeleteSub", "Remove files and folders", "删除文件和文件夹", "ファイルとフォルダーの削除", "파일과 폴더 삭제"),
    ("harnessPermissionExecute", "Run commands", "执行命令", "コマンド実行", "명령 실행"),
    ("harnessPermissionExecuteSub", "Shell commands, builds and scripts", "终端命令、构建和脚本", "シェルコマンド・ビルド・スクリプト", "셸 명령, 빌드, 스크립트"),
    ("harnessPermissionNetwork", "Network", "网络", "ネットワーク", "네트워크"),
    ("harnessPermissionNetworkSub", "Reach services on the internet", "访问互联网服务", "インターネット上のサービスへの接続", "인터넷 서비스 접속"),
    ("harnessPermissionGit", "Git", "Git", "Git", "Git"),
    ("harnessPermissionGitSub", "Commits, branches and history", "提交、分支和历史", "コミット・ブランチ・履歴", "커밋, 브랜치, 기록"),
    ("harnessPermissionGithub", "GitHub", "GitHub", "GitHub", "GitHub"),
    ("harnessPermissionGithubSub", "Issues, pull requests and actions", "议题、拉取请求和流水线", "イシュー・プルリクエスト・アクション", "이슈, 풀 리퀘스트, 액션"),
    ("harnessPermissionBrowser", "Browse", "浏览", "ブラウズ", "브라우징"),
    ("harnessPermissionBrowserSub", "Fetch pages and download files", "抓取网页和下载文件", "ページ取得とファイルのダウンロード", "페이지 가져오기와 파일 다운로드"),
    ("harnessPermissionSensitive", "Outside the workspace", "工作区之外", "ワークスペース外", "작업 공간 밖"),
    ("harnessPermissionSensitiveSub", "Touch anything beyond the workspace folder", "触碰工作区文件夹之外的任何内容", "ワークスペース外の領域へのアクセス", "작업 공간 폴더 밖 접근"),
    ("harnessPermissionDevice", "Device control", "设备控制", "端末操作", "기기 제어"),
    ("harnessPermissionDeviceSub", "Screen, taps, apps and notifications", "屏幕、点击、应用和通知", "画面・タップ・アプリ・通知", "화면, 탭, 앱, 알림"),
    ("harnessPolicyAllow", "Allow", "允许", "許可", "허용"),
    ("harnessPolicyConfirm", "Ask", "询问", "確認", "확인"),
    ("harnessPolicyDeny", "Block", "禁止", "禁止", "차단"),
    ("harnessRunTool(name: String)", "Run {name}", "执行 {name}", "{name} を実行", "{name} 실행"),
    ("harnessRunToolOn(name: String, detail: String)", "Run {name}: {detail}", "执行 {name}：{detail}", "{name} を実行：{detail}", "{name} 실행: {detail}"),
    ("reasoningDefaultLabel", "Provider default", "服务商默认", "プロバイダー既定", "제공자 기본값"),
    ("settingsAgentTitle", "Toolkit", "工具箱", "道具箱", "도구함"),
    ("settingsAgentSub", "Files, documents, commands, plugins and permissions", "文件、文档、命令、插件与权限", "ファイル・文書・コマンド・プラグイン・権限", "파일, 문서, 명령, 플러그인, 권한"),
    ("agentToolkitTitle", "Toolkit", "工具箱", "道具箱", "도구함"),
    ("agentToolkitSub", "What the assistant may do beyond notes and tasks", "助手在笔记和任务之外可以做的事", "メモとタスク以外にできること", "노트와 할 일 외에 할 수 있는 일"),
    ("agentToolkitEnabled", "Toolkit on", "启用工具箱", "道具箱を有効化", "도구함 사용"),
    ("agentToolkitEnabledSub", "Give the assistant real file, document and shell tools", "让助手拥有真实的文件、文档和终端工具", "ファイル・文書・シェル操作を許可", "파일, 문서, 셸 도구 제공"),
    ("agentSubAgents", "Sub-agents", "子助手", "サブエージェント", "하위 에이전트"),
    ("agentSubAgentsSub", "Let the assistant spawn helpers for heavy tasks", "允许助手为繁重任务召唤帮手", "重い作業でヘルパーを起動", "무거운 작업에서 도우미 실행"),
    ("agentSnapshots", "Snapshots", "快照", "スナップショット", "스냅숏"),
    ("agentSnapshotsSub", "Keep a copy of every file before it is changed", "改动前保留每个文件的副本", "変更前のファイルを複製", "변경 전 파일 사본 보관"),
    ("agentDeviceControl", "Device control", "设备控制", "端末操作", "기기 제어"),
    ("agentDeviceControlSub", "Read the screen, tap, type and open apps", "读屏、点击、输入和打开应用", "画面読み取り・タップ・入力・アプリ起動", "화면 읽기, 탭, 입력, 앱 실행"),
    ("agentEnableAccessibility", "Turn on screen access", "开启屏幕访问", "画面アクセスを有効にする", "화면 접근 켜기"),
    ("agentGrantStorage", "Allow all files", "允许访问所有文件", "すべてのファイルへのアクセスを許可", "모든 파일 접근 허용"),
    ("agentWorkspaceTitle", "Workspace", "工作区", "ワークスペース", "작업 공간"),
    ("agentWorkspaceSub", "The folder the assistant may read and write", "助手可以读写的文件夹", "読み書きするフォルダー", "읽고 쓰는 폴더"),
    ("agentWorkspaceLabel", "Folder", "文件夹", "フォルダー", "폴더"),
    ("agentPermissionsTitle", "Permissions", "权限", "権限", "권한"),
    ("agentPermissionsSub", "Tap a permission to cycle allow, ask and block", "点击权限在允许、询问、禁止之间切换", "タップで許可・確認・禁止を切り替え", "탭하여 허용, 확인, 차단 전환"),
    ("agentGroupsTitle", "Tool groups", "工具分组", "ツールグループ", "도구 그룹"),
    ("agentGroupsSub", "Switch off anything you would rather the assistant never used", "关掉你不希望助手使用的工具", "使わせたくないツールを無効化", "쓰지 않았으면 하는 도구 끄기"),
    ("agentSandboxTitle", "Execution", "执行方式", "実行方法", "실행 방식"),
    ("agentSandboxSub", "Where commands run and how long they may take", "命令在哪里运行以及可以运行多久", "コマンドの実行先と制限時間", "명령 실행 위치와 제한 시간"),
    ("agentTimeout", "Timeout", "超时", "タイムアウト", "시간 초과"),
    ("agentSubAgentLimit", "Helpers at once", "同时子助手数", "同時ヘルパー数", "동시 도우미 수"),
    ("agentGithubTitle", "GitHub", "GitHub", "GitHub", "GitHub"),
    ("agentGithubSub", "Used for issues, pull requests and reading build logs", "用于议题、拉取请求和读取构建日志", "イシュー・PR・ビルドログに使用", "이슈, 풀 리퀘스트, 빌드 로그"),
    ("agentGithubToken", "Access token", "访问令牌", "アクセストークン", "액세스 토큰"),
    ("agentPluginsTitle", "Plugins", "插件", "プラグイン", "플러그인"),
    ("agentPluginsSub", "Download extra tools such as a Linux userland, Python or LibreOffice", "下载 Linux 环境、Python、LibreOffice 等额外工具", "Linux 環境・Python・LibreOffice などを追加", "Linux 환경, Python, LibreOffice 등 추가 도구"),
    ("agentPluginInstalled", "Installed", "已安装", "インストール済み", "설치됨"),
    ("agentPluginInstall", "Download", "下载", "ダウンロード", "내려받기"),
    ("agentPluginRemove", "Remove", "移除", "削除", "제거"),
    ("agentPluginNeedsShell", "Needs Termux on Android", "安卓上需要 Termux", "Android では Termux が必要", "Android에서는 Termux 필요"),
    ("agentPluginSize(size: String)", "{size}", "{size}", "{size}", "{size}"),
    ("agentPluginSource(label: String)", "Source: {label}", "来源：{label}", "取得元：{label}", "출처: {label}"),
    ("agentPluginSpeed(bytes: String)", "{bytes}/s", "{bytes}/秒", "{bytes}/秒", "{bytes}/초"),
    ("agentMcpTitle", "MCP servers", "MCP 服务器", "MCP サーバー", "MCP 서버"),
    ("agentMcpSub", "Connect outside tools so the assistant can use them", "接入外部工具供助手使用", "外部ツールを接続して使う", "외부 도구를 연결해 사용"),
    ("agentMcpEmpty", "No servers yet", "还没有服务器", "サーバーはまだありません", "아직 서버가 없습니다"),
    ("agentMcpAdd", "Add a server", "添加服务器", "サーバーを追加", "서버 추가"),
    ("agentMcpName", "Name", "名称", "名前", "이름"),
    ("agentMcpUrl", "URL", "URL", "URL", "URL"),
    ("agentMcpCommand", "Command", "命令", "コマンド", "명령"),
    ("agentMcpToken", "Token", "令牌", "トークン", "토큰"),
    ("agentMcpAddAction", "Add", "添加", "追加", "추가"),
    ("agentConnectorsTitle", "Connectors", "连接器", "コネクター", "커넥터"),
    ("agentConnectorsSub", "Tokens for the services the assistant may use", "助手可以使用的服务令牌", "assistant が使えるサービスのトークン", "assistant가 사용할 서비스 토큰"),
    ("agentConnectorToken", "Token", "令牌", "トークン", "토큰"),
    ("agentConnectorAccount", "Account or workspace", "账号或工作区", "アカウントまたはワークスペース", "계정 또는 작업 공간"),
    ("agentConnectorBase", "Site URL", "站点地址", "サイト URL", "사이트 주소"),
    ("agentConnectorEmpty", "not set", "未设置", "未設定", "설정 안 됨"),
    ("agentConnectorSet", "configured", "已配置", "設定済み", "설정됨"),
    ("agentScreenAccess", "Screen access", "开启屏幕访问", "画面アクセス", "화면 접근"),
    ("agentScreenAccessSub", "Let the assistant read the screen and tap for you", "允许助手读屏并代为点击", "画面の読み取りとタップを許可", "화면 읽기와 탭 허용"),
    ("agentToolkitOffHint", "Nothing below is in use until the toolkit is switched on.", "未启用工具箱前，下面的设置都不生效。", "道具箱を有効にするまで、以下の設定は使われません。", "도구함을 켜기 전에는 아래 설정이 적용되지 않습니다."),
    ("agentCapabilitiesTitle", "Assistant abilities", "助手能力", "アシスタントの機能", "어시스턴트 기능"),
    ("agentCapabilitiesSub", "Sub-agents, snapshots, device control and screen access", "子助手、快照、设备控制与屏幕访问", "サブエージェント・スナップショット・端末操作・画面アクセス", "하위 에이전트, 스냅숏, 기기 제어, 화면 접근"),
    ("agentWorkspacePick", "Choose folder", "选择文件夹", "フォルダーを選択", "폴더 선택"),
    ("agentWorkspacePickUnavailable", "This build cannot open a folder picker.", "此版本无法打开文件夹选择器。", "このビルドではフォルダー選択を開けません。", "이 빌드에서는 폴더 선택기를 열 수 없습니다."),
    ("agentWorkspacePickTitle", "Select workspace folder", "选择工作区文件夹", "ワークスペースフォルダーを選択", "작업 공간 폴더 선택"),
    ("agentWorkspaceNewFolder", "New folder", "新建文件夹", "新しいフォルダー", "새 폴더"),
    ("agentWorkspaceNewFolderTitle", "Folder name", "文件夹名称", "フォルダーの名前", "폴더 이름"),
    ("agentWorkspaceShowHidden", "Show hidden files", "显示隐藏文件", "隠しファイルを表示", "숨김 파일 표시"),
    ("agentWorkspaceUp", "Up one level", "上一级", "一つ上へ", "상위 폴더"),
    ("agentWorkspaceHome", "Home", "主目录", "ホーム", "홈"),
    ("agentWorkspaceEmpty", "No folders here", "这里没有文件夹", "フォルダーがありません", "폴더가 없습니다"),
    ("agentWorkspaceUnreadable", "This folder cannot be listed", "无法列出这个文件夹", "このフォルダーを一覧できません", "이 폴더를 나열할 수 없습니다"),
    ("agentWorkspaceEditPath", "Edit path", "编辑路径", "パスを編集", "경로 편집"),
    ("settingsSaved", "Saved", "已保存", "保存しました", "저장됨"),
    ("agentGithubTokenName(index: Int)", "Token {index}", "令牌{index}", "トークン{index}", "토큰 {index}"),
    ("agentGithubTokensHint", "Up to five tokens. Tap one to choose the one the assistant uses.", "最多五个令牌，点一下即可指定助手使用哪一个。", "最大5件。タップすると assistant が使うトークンを選べます。", "최대 5개이며, 탭하면 assistant가 사용할 토큰을 고릅니다."),
    ("agentGithubRename", "Rename", "重命名", "名前を変更", "이름 바꾸기"),
    ("agentGithubRenameTitle", "Token name", "令牌名称", "トークン名", "토큰 이름"),
    ("agentGithubStored", "Saved. The token will not be shown again.", "已保存，之后不再显示该令牌。", "保存しました。以後このトークンは表示されません。", "저장되었습니다. 이 토큰은 다시 표시되지 않습니다."),
    ("agentGithubRemoveToken", "Remove this token", "移除该令牌", "このトークンを削除", "이 토큰 제거"),
    ("pluginInstallFailed", "Plugin installation failed", "插件安装失败", "プラグインのインストールに失敗", "플러그인 설치 실패"),
    ("pluginReasonNoShell", "Lucent has no shell on this device yet, so it can download but cannot unpack or run anything.", "Lucent 在这台设备上还没有可用的 shell，因此只能下载，无法解包或运行。", "この端末にはまだシェルがないため、ダウンロードはできても展開や実行はできません。", "이 기기에는 아직 셸이 없어 내려받기만 되고 압축을 풀거나 실행할 수 없습니다."),
    ("pluginReasonDownload", "The download did not finish, so nothing was installed.", "下载没有完成，因此没有安装任何内容。", "ダウンロードが完了しなかったため、何もインストールされていません。", "내려받기가 끝나지 않아 아무것도 설치되지 않았습니다."),
    ("pluginReasonInstall", "The install command stopped with an error.", "安装命令出错中止了。", "インストールコマンドがエラーで停止しました。", "설치 명령이 오류로 중단되었습니다."),
    ("pluginReasonDetect", "The install command finished, but the plugin still cannot be found.", "安装命令结束了，但仍然找不到该插件。", "インストールコマンドは終了しましたが、プラグインが見つかりません。", "설치 명령은 끝났지만 플러그인을 찾을 수 없습니다."),
    ("pluginReasonUnsupported", "This plugin cannot be installed on this platform.", "这个平台无法安装该插件。", "このプラットフォームにはこのプラグインをインストールできません。", "이 플랫폼에는 이 플러그인을 설치할 수 없습니다."),
    ("pluginRepairTitle", "How to fix it", "修复建议", "対処方法", "해결 방법"),
    ("pluginRepairTermux", "Install Termux from F-Droid, open it once, run termux-setup-storage, and set allow-external-apps=true in ~/.termux/termux.properties.", "从 F-Droid 安装 Termux，打开一次，运行 termux-setup-storage，并在 ~/.termux/termux.properties 中设置 allow-external-apps=true。", "F-Droid から Termux を入れ、一度起動して termux-setup-storage を実行し、~/.termux/termux.properties に allow-external-apps=true を設定してください。", "F-Droid에서 Termux를 설치하고 한 번 실행해 termux-setup-storage를 실행한 뒤 ~/.termux/termux.properties에 allow-external-apps=true를 설정하세요."),
    ("pluginRepairStorage", "Grant \u201cAllow all files\u201d on the Workspace page so the shell can read what Lucent downloaded.", "在工作区页面授予“允许访问所有文件”，shell 才能读取 Lucent 下载的文件。", "ワークスペース画面で「すべてのファイルへのアクセスを許可」を有効にすると、シェルがダウンロードしたファイルを読めます。", "작업 공간 화면에서 \"모든 파일 접근 허용\"을 켜야 셸이 내려받은 파일을 읽을 수 있습니다."),
    ("pluginRepairUserland", "Install the Linux userland first: this plugin is built inside it.", "请先安装 Linux 环境插件，这个插件是在其中构建的。", "先に Linux ユーザーランドを入れてください。このプラグインはその中に構築されます。", "먼저 Linux 사용자 환경을 설치하세요. 이 플러그인은 그 안에 설치됩니다."),
    ("pluginRepairRetry", "Read the shell output below, fix the first error it names, then try again.", "查看下方的 shell 输出，解决它指出的第一个错误后重试。", "下のシェル出力を読み、最初のエラーを解消してから再試行してください。", "아래 셸 출력을 보고 첫 번째 오류를 해결한 뒤 다시 시도하세요."),
    ("pluginShellNone", "No shell yet — downloading works, installing and running need Termux.", "还没有 shell——可以下载，安装和运行需要 Termux。", "シェルがありません。ダウンロードは可能ですが、インストールと実行には Termux が必要です。", "셸이 없습니다. 내려받기는 되지만 설치와 실행에는 Termux가 필요합니다."),
    ("pluginShellReady(shell: String)", "Installing and running through {shell}.", "通过 {shell} 安装和运行。", "{shell} 経由でインストールと実行を行います。", "{shell}을(를) 통해 설치하고 실행합니다."),
    ...
    ("backupModHarness", "Assistant data", "助手数据", "アシスタントデータ", "어시스턴트 데이터"),
    ("backupModHarnessDesc(size: String)", "Includes the assistant's own files \u2014 todos, memory, skills, snapshots and its activity log ({size}) \u2014 so a restore brings them back too.", "包含助手自身的文件——待办、记忆、技能、快照与活动日志（{size}），还原时一并恢复。", "アシスタント自身のファイル（ToDo、記憶、スキル、スナップショット、動作ログ、{size}）を含めるため、復元時も戻ります。", "어시스턴트 자체 파일(할 일, 기억, 스킬, 스냅샷, 활동 로그, {size})을 포함하므로 복원 시 함께 돌아옵니다."),
    ("backupHarnessFilesInFile(count: Int)", "{count} assistant file(s)", "{count} 个助手文件", "アシスタントファイル {count} 件", "어시스턴트 파일 {count}개"),
    ("backupHarnessFilesRestored(count: Int)", "Restored {count} assistant file(s).", "已还原 {count} 个助手文件。", "アシスタントファイル {count} 件を復元しました。", "어시스턴트 파일 {count}개를 복원했습니다."),
    ("backupHarnessFilesFailed(count: Int)", "{count} assistant file(s) could not be restored.", "有 {count} 个助手文件未能还原。", "アシスタントファイル {count} 件を復元できませんでした。", "어시스턴트 파일 {count}개를 복원하지 못했습니다."),
    ("goalPanelTitle", "Goal", "目标", "目標", "목표"),
    ("goalEmpty", "No goal is set for this conversation.", "此对话还没有目标。", "この会話にはまだ目標がありません。", "이 대화에는 아직 목표가 없습니다."),
    ("goalPhaseActive", "Active", "进行中", "進行中", "진행 중"),
    ("goalPhasePaused", "Paused", "已暂停", "一時停止中", "일시 중지됨"),
    ("goalPhaseComplete", "Complete", "已完成", "完了", "완료"),
    ("goalPhaseBlocked", "Blocked", "受阻", "ブロック中", "차단됨"),
    ("goalPause", "Pause", "暂停", "一時停止", "일시 중지"),
    ("goalResume", "Resume", "继续", "再開", "재개"),
    ("goalComplete", "Mark complete", "标记为完成", "完了にする", "완료로 표시"),
    ("goalRounds(used: Int, limit: Int)", "Round {used} of {limit}", "第 {used} 轮，共 {limit} 轮", "{used} / {limit} ラウンド", "{limit}라운드 중 {used}라운드"),
    ("goalRoundNote(round: Int, limit: Int)", "Goal round {round} of {limit}", "目标第 {round} 轮，共 {limit} 轮", "目標ラウンド {round} / {limit}", "목표 {limit}라운드 중 {round}라운드"),
    ("goalBlocker(reason: String)", "Blocked: {reason}", "受阻：{reason}", "ブロック：{reason}", "차단됨: {reason}"),
    ("agentContextBudget", "Context budget", "上下文预算", "コンテキスト予算", "컨텍스트 예산"),
    ("agentContextBudgetSent(tokens: String)", "Context: {tokens} sent", "上下文：已发送 {tokens}", "コンテキスト：{tokens} 送信済み", "컨텍스트: {tokens} 전송됨"),
    ("agentContextBudgetDropped(count: Int)", "{count} older turns summarised", "已概括 {count} 轮较早对话", "{count} 件の古いやり取りを要約", "이전 {count}개 대화를 요약함"),
    ("agentContextBudgetTrimmed(count: Int)", "{count} tool results shortened", "已缩短 {count} 条工具结果", "{count} 件のツール結果を短縮", "도구 결과 {count}개를 줄임"),
    ("actionStop", "Stop", "停止", "停止", "중지"),
    ("jobsPanelTitle", "Background jobs", "后台作业", "バックグラウンドジョブ", "백그라운드 작업"),
    ("jobsRunningLabel", "Running now", "正在运行", "実行中", "실행 중"),
    ("jobsRunning", "running", "运行中", "実行中", "실행 중"),
    ("jobsFinished", "finished", "已完成", "終了", "완료"),
    ("jobsEmpty", "No background jobs yet.", "还没有后台作业。", "バックグラウンドジョブはまだありません。", "아직 백그라운드 작업이 없습니다."),
    ("todoPanelTitle", "Todo list", "待办清单", "ToDo リスト", "할 일 목록"),
    ("todoSectionTodo", "Todo", "待办", "ToDo", "할 일"),
    ("todoSectionPlan", "Plan", "计划", "計画", "계획"),
    ("todoEmpty", "No todos yet.", "还没有待办。", "ToDo はまだありません。", "아직 할 일이 없습니다."),
    ("todoPlanEmpty", "No plan yet.", "还没有计划。", "計画はまだありません。", "아직 계획이 없습니다."),
    ("todoStatusPending", "pending", "待办", "待機中", "대기 중"),
    ("todoStatusActive", "in progress", "进行中", "進行中", "진행 중"),
    ("todoStatusDone", "done", "已完成", "完了", "완료"),
    ("subAgentSend", "Send", "发送", "送信", "보내기"),
    ("subAgentStop", "Stop", "停止", "停止", "중지"),
    ("subAgentInbox", "queued instruction", "待送达的指令", "送信待ちの指示", "전달 대기 지시"),
    ("subAgentInstructHint", "Add an instruction for this helper", "给这个帮手追加一条指令", "このヘルパーへの指示を追加", "이 도우미에게 지시 추가"),
    ("subAgentEmpty", "No sub-agent has run yet.", "还没有运行过子助手。", "サブエージェントはまだ実行されていません。", "아직 실행된 하위 에이전트가 없습니다."),
    ("runningCounter(jobs: Int, agents: Int)", "{jobs} jobs \u00b7 {agents} helpers", "{jobs} 个作业 \u00b7 {agents} 个帮手", "{jobs} ジョブ \u00b7 {agents} ヘルパー", "{jobs}개 작업 \u00b7 {agents}개 도우미"),

    ("agentTermuxTitle", "Termux", "Termux", "Termux", "Termux"),
    ("agentTermuxSub", "The terminal app the assistant runs real commands in", "助手运行真实命令的终端应用", "assistant が実際のコマンドを実行するターミナルアプリ", "assistant가 실제 명령을 실행하는 터미널 앱"),
    ("agentTermuxAction", "Get Termux", "下载 Termux", "Termux を入手", "Termux 받기"),
    ("agentTermuxInstalled", "Termux is installed", "已安装 Termux", "Termux はインストール済み", "Termux 설치됨"),
    ("shizukuForAssistant", "Shizuku for the assistant", "给予助手 Shizuku 权限", "assistant に Shizuku 権限を与える", "assistant에 Shizuku 권한 부여"),
    ("shizukuForAssistantSub", "Let the assistant run commands through Shizuku. Off by default.", "允许助手通过 Shizuku 执行命令。默认关闭。", "assistant が Shizuku 経由でコマンドを実行できるようにします。既定はオフです。", "assistant가 Shizuku를 통해 명령을 실행하도록 허용합니다. 기본값은 꺼짐입니다."),
    ("subAgentStatusRunning", "working", "进行中", "実行中", "작업 중"),
    ("subAgentStatusDone", "finished", "已完成", "完了", "완료"),
    ("subAgentStatusFailed", "failed", "失败", "失敗", "실패"),
    ("subAgentRounds(rounds: Int)", "{rounds} rounds", "{rounds} 轮", "{rounds} ラウンド", "{rounds}회"),
    ("subAgentWork", "Work", "过程", "作業", "작업"),
    ("subAgentResult", "Result", "结果", "結果", "결과"),
    ("subAgentSaveReport", "Save report", "保存报告", "レポートを保存", "보고서 저장"),
    ("subAgentReportSaved(name: String)", "Saved to {name}", "已保存到 {name}", "{name} に保存しました", "{name}에 저장됨"),
    ("subAgentReportFolder(folder: String)", "Sub-agent reports are written to {folder} in the workspace.", "子代理报告写入工作区的 {folder}。", "サブエージェントのレポートはワークスペースの {folder} に書き込まれます。", "하위 에이전트 보고서는 작업 공간의 {folder}에 저장됩니다."),
    ("agentAuditTitle", "Activity log", "操作记录", "操作ログ", "작업 기록"),
    ("agentAuditSub", "Every tool the assistant has run", "助手运行过的每一个工具", "実行したすべてのツール", "실행한 모든 도구"),
    ("agentAuditEmpty", "Nothing has run yet", "还没有运行记录", "まだ実行はありません", "아직 실행 기록이 없습니다"),
    ("actionShowMore", "Show more", "显示更多", "もっと見る", "더 보기"),
