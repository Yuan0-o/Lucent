...
ENTRIES = [
    ...
    ("aboutLicenseFull", "See THIRD-PARTY-NOTICES.md for full details.", "详见 THIRD-PARTY-NOTICES.md。", "詳細はTHIRD-PARTY-NOTICES.mdをご覧ください。", "자세한 내용은 THIRD-PARTY-NOTICES.md를 참조하세요."),
    ...
    ("lmGpuSub", "Hand the model to the GPU when the device allows it", "设备允许时把模型交给 GPU 运行", "端末が許せばモデルを GPU に任せます", "기기가 허락하면 모델을 GPU에 맡깁니다"),
    ("lmToolsSub", "Let it create, edit and complete your notes and tasks", "允许它新建、修改和完成你的笔记与任务", "ノートとタスクの作成・編集・完了を許します", "노트와 할 일의 생성·수정·완료를 허용합니다"),
    ("lmWebSearchSub", "Let the local model search the web while agent mode is on. Off by default.", "允许本地模型在 Agent 模式下联网查找资料，默认关闭。", "エージェントモード中にローカルモデルのウェブ検索を許します。既定はオフ。", "에이전트 모드에서 로컬 모델의 웹 검색을 허용합니다. 기본은 꺼짐."),
    ("lmWebSearchToggle", "Web search", "网络搜索", "ウェブ検索", "웹 검색"),
    ("lmBackgroundSub", "Keep replying after you leave the app", "离开软件后继续回复", "アプリを離れても返信を続けます", "앱을 나가도 계속 답합니다"),
